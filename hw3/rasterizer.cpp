...
void rst::rasterizer::rasterize_triangle(const Triangle& t, const std::array<Eigen::Vector3f, 3>& view_pos) 
{
    ...
    for (int x = min_x; x <= max_x; x++)
        ...
        {
            if (insideTriangle(x + 0.5, y + 0.5, t.v)) // 此像素在三角形内部
            {
                // 插值
                // Barycentric Coordinates
                auto [alpha, beta, gamma] = computeBarycentric2D(x + 0.5, y + 0.5, t.v);
                //float alpha = (-(x - v[1][0]) * (v[2][1] - v[1][1]) + (y - v[1][1]) * (v[2][0] - v[1][0]))\
                //    / (-(v[0][0] - v[1][0]) * (v[2][1] - v[1][1]) + (v[0][1] - v[1][1]) * (v[2][0] - v[1][0]));
                //float beta = (-(x - v[2][0]) * (v[0][1] - v[2][1]) + (y - v[2][1]) * (v[0][0] - v[2][0]))\
                //    / (-(v[1][0] - v[2][0]) * (v[0][1] - v[2][1]) + (v[1][1] - v[2][1]) * (v[0][0] - v[2][0]));
                //float gamma = 1 - alpha - beta;
                // depth
                float Z = 1.0 / (alpha / v[0].w() + beta / v[1].w() + gamma / v[2].w());
                float zp = alpha * v[0].z() / v[0].w() + beta * v[1].z() / v[1].w() + gamma * v[2].z() / v[2].w();
                zp *= Z;
                if (zp < depth_buf[get_index(x + 0.5, y + 0.5)])
                {
                    depth_buf[get_index(x + 0.5, y + 0.5)] = zp;
                    ...
                    set_pixel(Vector2i(x + 0.5, y + 0.5), pixel_color);
                }
            }
        } 
